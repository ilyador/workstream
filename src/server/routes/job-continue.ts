...
jobContinueRouter.post('/api/jobs/:id/continue', requireAuth, async (req, res) => {
  ...
  const { error: jobUpdateErr } = await supabase
    .from('jobs')
    .update({ status: 'queued', question: null, log_offset: logOffset, flow_snapshot: newSnapshot })
    .eq('id', jobId);
  ...
  if (taskUpdateErr) {
    const { error: rollbackErr } = await supabase
      ...
      .update({
        ...
        log_offset: job.log_offset ?? null,
        flow_snapshot: job.flow_snapshot ?? null,
      })
      .eq('id', jobId);
    if (rollbackErr) console.error(`[jobs] Failed to roll back continue for job ${jobId}:`, rollbackErr.message);
