...
export function buildFlowSnapshot(flow: unknown): FlowConfig {
  ...
  const steps = rawSteps
    ...
    .map((step, index) => {
      const stepName = stringValue(step.name, `step ${index + 1}`);
      const explicitModel = typeof step.model === 'string' ? step.model.trim() : '';
      const model = explicitModel || (providerBinding === 'task_selected' ? 'task:selected' : '');
      if (!model) {
