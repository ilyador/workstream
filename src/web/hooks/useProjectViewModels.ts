...
export function useProjectViewModels({
  ...
  const jobViews: JobView[] = useMemo(() => {
    ...
    return sorted.map(job => ({
      ...
      phases: buildPhases(job.phases_completed || [], job.current_phase, taskTypeMap[job.task_id] || 'feature', job.flow_snapshot),
